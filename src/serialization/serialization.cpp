...
YAML::Node serialize_entity(entt::registry& reg, entt::entity e) {
    ...
    for (const auto& type_name : ComponentSerializer::get_registered_types()) {
        auto yaml_str = ComponentSerializer::serialize(type_name, reg, e);
        if (!yaml_str.empty()) {
            YAML::Node comp;
            comp["id"] = type_name;
            comp["params"] = yaml_str;
...
void deserialize_entity(const YAML::Node& node, entt::registry& reg) {
    auto entity_id = node["id"].as<uint32_t>();
    auto entity = reg.create(static_cast<entt::entity>(entity_id));
