...
void UITileLayoutComp::OnResize(BoundsResizeEvent& ev) {
    entt::registry& reg = *ev.registry;
    entt::entity entity = ev.entity;
    ...
    int cols = std::max(1, (int)((contentWidth + spacing) / (tileSize.x + spacing)));

    int col = 0;
    float cursorX = contentLeft;
    float cursorY = contentTop - tileSize.y;
    float minY = cursorY;
    float maxX = cursorX;

    for (entt::entity child : children) {
        sf::Vector2f pos(cursorX, cursorY);
        sf::FloatRect cBounds(pos + ui.childOffset, tileSize);
        raise_local_event(reg, child, UISizeAllocatedEvent{cBounds, &reg, child});

        col++;
        if (col >= cols) {
            col = 0;
            cursorX = contentLeft;
            cursorY -= (tileSize.y + spacing);
        } else {
            cursorX += (tileSize.x + spacing);
        }
